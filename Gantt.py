...
class GanttCanvas(QWidget):
    ...
    def convX(self, x):
        return int(x * self._vwidth / float(self._end_date - self._start_date))
        ...
        for i in range(start_date, end_date + 1, 1):
            ...
            if i % step == 0:
                ...
                if i != start_date and i != end_date:
                    ...
                    qp.drawLine(x + int(convX(i - start_date)), y,
                                x + int(convX(i - start_date)),
                                graph_height + y + 1)
                    qp.setPen(pen)
            ...
            if h:
                qp.drawLine(
                    x + int(convX(i - start_date)), graph_height + 1 + y,
                    x + int(convX(i - start_date)), graph_height + y + 1 + h)
